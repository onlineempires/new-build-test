...
export default function TemplateSelector({ isOpen, onClose, onSelect }: TemplateSelectorProps) {
  ...
  // Compute available space based on AppLayout header/sidebar, like SalesCloser
  const [availableSpace, setAvailableSpace] = useState({ left: 0, top: 0, right: 0, bottom: 0 });
  useEffect(() => {
    const calculateSpace = () => {
      ...
      setAvailableSpace({ left: sidebarWidth, top: headerHeight + 20, right: 0, bottom: 0 });
    };
    calculateSpace();
  ...
  return (
    <div className="fixed inset-0 z-50">
      ...
      {/* Modal container centered within content area (offset by header/sidebar) */}
      <div
        className="fixed z-[51] flex items-center justify-center overflow-y-auto p-4"
        style={{
            ...
            <div className="flex items-center space-x-3">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full rounded-lg border border-gray-300 py-2 pl-8 pr-3 text-sm focus:border-transparent focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
              </div>
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                {filteredTemplates().map((template) => (
                  <div
                    key={template.id}
                    className="cursor-pointer rounded-xl border-2 border-transparent bg-gray-50 p-4 transition-all hover:border-blue-200 hover:shadow-lg"
                  ...
                  >
                    ...
                    <div className="relative mb-4 flex aspect-video items-center justify-center overflow-hidden rounded-lg bg-gradient-to-br from-blue-500 to-purple-600">
                      {template.preview_image_url ? (
                        ...
                        <div className="text-center text-white">
                          <Zap className="mx-auto mb-2 h-7 w-7" />
                          <p className="text-sm font-medium">Template Preview</p>
                        </div>
                      )}
                    ...
                    {/* Template Info */}
                    <div>
                      <h3 className="mb-2 text-sm font-semibold text-gray-900">{template.name}</h3>
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <button className="flex items-center space-x-1 text-xs font-medium text-blue-600 hover:text-blue-700">
