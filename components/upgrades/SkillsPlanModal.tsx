...
interface SkillsPlanModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export default function SkillsPlanModal({ 
  isOpen, 
  onClose, 
  currentPlan = 'free',
  onUpgradeSuccess 
}: SkillsPlanModalProps) {
  ...
  const planDetails = {
    'one-time': {
      ...
      description: '6 months access to courses',
      tag: 'Best Value',
      sku: 'SKU_SKILLS_499',
      features: [
    ...
    'monthly': {
      ...
      price: '$99',
      subtitle: '/ month',
      description: 'Monthly skills access',
      sku: 'SKU_SKILLS_99_MONTH',
      features: [
        'All course catalog access',
        'Download materials',
        'Billed monthly',
      ...
      if (paymentMethod === 'apple-pay') {
        const paymentRequest = {
          ...
          total: {
            ...
            amount: selectedPlan === 'one-time' ? '499.00' : '99.00'
          }
        };
        ...
        setTimeout(() => {
          setIsProcessing(false);
          // Grant Skills Plan entitlements (courses only, exclude masterclasses)
          localStorage.setItem('userRole', 'skills');
          localStorage.setItem('purchaseData', JSON.stringify({
            plan: selectedPlan,
            sku: planDetails[selectedPlan].sku,
            entitlements: ['courses'], // No masterclasses
            accessType: selectedPlan === 'one-time' ? '6months' : 'monthly',
            purchaseDate: new Date().toISOString(),
            expiryDate: selectedPlan === 'one-time' ? 
              new Date(Date.now() + 6 * 30 * 24 * 60 * 60 * 1000).toISOString() : null
          }));
          onClose();
        ...
        setTimeout(() => {
          setIsProcessing(false);
          // Grant Skills Plan entitlements
          localStorage.setItem('userRole', 'skills');
          localStorage.setItem('purchaseData', JSON.stringify({
            plan: selectedPlan,
            sku: planDetails[selectedPlan].sku,
    ...
    <div className="fixed inset-0 bg-black/60 z-[1050] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white w-[92vw] max-w-[720px] rounded-3xl p-6 md:p-8 shadow-2xl z-[1060] max-h-[90vh] overflow-y-auto">
        ...
        <div className="mb-6">
          ...
          <p className="text-gray-600">Get access to our complete course catalog</p>
        </div>
                ...
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1">
                    ...
                    <div className="flex items-baseline gap-1">
                      ...
                      {plan.subtitle && <span className="text-gray-600">{plan.subtitle}</span>}
                    </div>
        ...
        <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4 mb-6">
          <div className="flex items-center">
            ...
            <div>
              <p className="text-sm font-medium text-yellow-800">Masterclasses not included</p>
              <p className="text-xs text-yellow-700">Masterclasses are included in Premium Annual membership</p>
            </div>
          </div>
        </div>
